...
func VaultInit(vaultConfig vault.Config) error {
	...
	log.Println("Creating fairsplit-api policy")

	err = vaultSrv.CreatePolicy(ctx, "fairsplit-api", "secret/data/*", []string{"read", "create", "update"})
	if err != nil {
	...
	log.Printf("Generating fairsplit-api token: %s", vaultConfig.Token)

	// We don't currently save the token because we're always going to specify it.
	err = vaultSrv.CreateToken(ctx, vaultConfig.Token, []string{"fairsplit-api"}, "Fairsplit API")
	if err != nil {
