// Package vault provides an implementation to support hashicorp and retrieve private keys
package vault
...
// defaultClient is a standard configuration. It should not be used in production env.
...
// Vault provides support to access Hashicorp's vault.
type Vault struct {
	address   string
	token     string
	mountPath string
...
// Config represent the required settings to work with the vault.
type Config struct {
	Address   string
	MountPath string
	Token     string
...
// SetToken allows the user to replace the token used on calls.
func (v *Vault) SetToken(token string) {
	v.token = token
}

// AddPrivateKey adds a new private key into the vault as PEM encoded.
...
// PrivateKey searches the key store for a given key id and returns
...
// PublicKey searches the key store for a given key id and returns
...
// Error variables to be used in the following lines.
var (
...
// Unseal replicates the unseal command behavior.
...
func (v *Vault) Mount(ctx context.Context) error {
	...
	// Mount already exists - do nothing.
